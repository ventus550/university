...
def get_square(y, x):
	y *= 3; x *= 3
	return [V(row, col) for row in range(y, y+3) for col in range(x, x+3)]
...
def squares():
    return [all_different(get_square(row, col)) for row in range(3) for col in range(3)]
...
# python3 sudoku.py > sud.pl && swipl -c sud.pl > sud_solved.txt && python3 test_sudoku.py
