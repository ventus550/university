...
def BFS(player):
    ...
    while not Q.empty():
        ...
        for d in directions:
            ...
            if MAP[cell] == VOID or MAP[cell] == UNKN or (player.health >= 15 and MAP[cell] == HPOT):
                # log(f"void if {x} {y}")
...
def fight(player):
    ...
    def scythe():
        best = None; bval = 0
        directions = [(-1, -1), (+1, +1), (-1, +1), (-1, +1)] # fix this!
        ...
        for d in directions:
            ...
            val = (ismonster(t0) == ismonster(t1) == True) * 2
...
def goto(player, target, ptr):
    ...
    while not np.array_equal(ptr[tx][ty], player.pos):
        tx, ty = ptr[tx][ty]
    MOVE(tx, ty, f"goto {target}")


def danger(target):
    x, y = target
    directions = (-2, -1, 0, 1, 2)
    return sum( ismonster(MAP[x + i, y + j]) for i in directions for j in directions )


def explore(player, ptr):
    log("explore!")
    try:
        log([(target, danger(target)) for target, type in mapscan() if type == UNKN and not ptr[target[0]][target[1]] is None])
        spot = min([target for target, type in mapscan() if type == UNKN and not ptr[target[0]][target[1]] is None], key=lambda t: danger(t)) #lambda x: dist[x[0]][x[1]])
...
def ACTION(player):
    ...
    if player.health <= 15:
        ...
        x, y = player.pos
        try:
            pots =  [target for target, type in mapscan() if type == HPOT]
            px, py = potion = min(pots, key=lambda t: dist[t[0]][t[1]])
            log("POT:", potion, dist[px][py] <= INF)
            if dist[px][py] <= INF:
                goto(player, potion, ptr)
                return
        except: ...
    ...
    if player.health >= 10 and TIME < 100:
