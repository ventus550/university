...
(struct environ (xs ys) #:transparent)

(define env-empty (environ null null))
(define (env-add x v env)
  (environ (cons (mcons x v) (environ-xs env)) (environ-ys env)))
(define (genv-add x v env)
  (environ (environ-xs env) (cons (mcons x v) (environ-ys env))))
(define (env-lookup x env)
  ...
  (assoc-lookup (append (environ-xs env) (environ-ys env))))
...
(define (env-update x v xs) ; <---------------------------------- !!!
  ...
  (environ (assoc-update (environ-xs xs)) (environ-ys xs)))
(define (localized env)
  (environ null (append (environ-xs env) (environ-ys env))))
...
(define (eval-env e env)
  (match e
    ...
    [(binop op l r) ((op->proc op) (eval-env l env)
                                   (eval-env r env))]
    [(let-expr x e1 e2)
     (eval-env e2 (env-add x (eval-env e1 env) (localized env)))]
    [(letrec-expr x e1 e2)
     (let* ([new-env (env-add x (blackhole) (localized env))]
    ...
    [(app f e)
     (let ([vf (eval-env f env)]
       ...
       (match vf [(clo x body fun-env)
                  (eval-env body (env-add x ve (localized fun-env)))]))]))
