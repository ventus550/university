...
(: binop-type (-> (U EType #f) (U EType #f) (-> (U EType #f) (U EType #f) (U EType #f))))
(define (binop-type l r)
  (lambda (operand-type true-type) (if (and (eq? l operand-type) (eq? r operand-type)) true-type #f)))
...
(define (typecheck-env e env)
  (match e
    ...
    [(binop op l r) (let* ([lv (typecheck-env l env)] [rv (typecheck-env r env)]
                           [bitype (binop-type lv rv)])
                                                     
                      (cond
                        [(member op (list '+ '- '* '/ '%)) (bitype 'real 'real)]
                        [(member op (list '= '> '>= '< '<=)) (bitype 'real 'boolean)]
                        [(member op (list 'and 'or)) (bitype 'boolean 'boolean)]
                        [else #f]))]
