...
(struct apply      (ls xs)    #:transparent)
...
(define (expr? e)
  (match e
    ...
    [(apply ls xs) (and (expr? ls) (expr? xs))]
    [_ false]))
  ...
  (cond
    ...
    [(and (list? q) (eq? (first q) 'list))
     (let loop ([xs (cdr q)])
       (if (null? xs)
           (null-expr)
           (cons-expr  (parse (car xs)) (loop [cdr xs]))))]
    ...
    [(and (list? q) (pair? q) (not (op->proc (car q)))) ; <------- !!!
     (parse-app q)]  ;<====jedyne uzycie parse-app
...
(define (eval-env e env)
  ...
  (match e
    ...
    [(apply ls xs)
      (let loop ([ls ls] [xs (eval-env xs env)] [env env])
        
        (define (app-list f val)
          (let ([vf (eval-env f env)])
            (match vf [(clo x body fun-env)
                       (if (value? body)
                           body
                           (eval-env body (env-add x val fun-env)))])))
        
        
        (if (null? xs)
            (eval-env ls env)
            (app-list (lam (lam-id ls) (loop [lam-e ls] [cdr xs] [env-add (lam-id ls) (car xs) env])) (car xs))))]
