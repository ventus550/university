...
(define (eval-env e env)
  (match e
    ...
    [(var-expr x) (let ([ex (env-lookup x env)])
                    (match ex
                      [(dly ex env) (eval-env ex env)]
                      [_ ex]))];;(env-lookup x env)
    [(if-expr eb et ef) (if (eval-env eb env)
                            (eval-env et env)
                            (eval-env ef env))]
    [(cons-expr e1 e2) (cons (dly e1 env)
                             (dly e2 env))]
